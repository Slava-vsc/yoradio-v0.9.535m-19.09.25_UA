...
void Display::_buildPager(){
  ...
  #endif
  #ifndef HIDE_HEAPBAR
    _heapbar = new SliderWidget(heapbarConf, config.theme.buffer, config.theme.background, UINT16_MAX * 5);
  #endif
  #ifndef HIDE_VOL
...
void Display::_apScreen() {
  if(_boot) _pager.removePage(_boot);
    ...
    bootTitle->setText("¸Radio AP Mode");
...
//    char *stitle = split(tmpbuf, " • ");
//    if (stitle == NULL) char *stitle = split(tmpbuf, " • ");
    if(stitle && _title2){
