...
// Заменяем "spiffs" на "littlefs" в проверке параметра и константу U_SPIFFS на U_FS
      int target = (request->getParam("updatetarget", true)->value() == "littlefs") ? U_SPIFFS : U_FLASH;
    ...
    if (!index) {
      String spath = "/www/";
	  // Если это плейлист или настройки Wi-Fi, сохраняем в папку /data/
      if(filename=="playlist.csv" || filename=="wifi.csv") spath = "/data/";
      request->_tempFile = LittleFS.open(spath + filename , "w");
    }
    if (len) {
      if (request->_tempFile) { // Добавляем проверку, что файл открыт
	  request->_tempFile.write(data, len);
	  }
    }
    if (final) {
