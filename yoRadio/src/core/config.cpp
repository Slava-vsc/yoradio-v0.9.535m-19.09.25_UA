...
void Config::init() {
  ...
  // Для ESP32 параметр (true) разрешает форматирование при ошибке монтирования
  if (!LittleFS.begin(true)) {
    Serial.println("##[ERROR]#\tLittleFS Mount Failed");
    return;
  }
  BOOTLOG("LittleFS mounted");
  // Проверка на пустоту (метод _isFSempty тоже должен внутри использовать LittleFS)
   emptyFS = _isFSempty();
  if(emptyFS) BOOTLOG("LittleFS is empty!");
  ssidsCount = 0;
  // Работа с указателями на файловую систему
  #ifdef USE_SD
  // Если режим SD — используем sdman, иначе — адрес объекта LittleFS
...
void Config::changeMode(int newmode){
  ...
  // Обновляем выбор файловой системы: если не SD-карта, то используем LittleFS
  _SDplaylistFS = getMode()==PM_SDCARD?&sdman:(true?&LittleFS:_SDplaylistFS);
  if(getMode()==PM_SDCARD){
...
bool Config::littlefsCleanup(){
  // Проверяем существование файлов перед удалением (используем LittleFS)
...
void Config::indexPlaylist() {
  // Открываем файл плейлиста для чтения из LittleFS
  ...
  int sOvol;
  // Создаем файл индекса для записи в LittleFS
  File index = LittleFS.open(INDEX_PATH, "w");
  while (playlist.available()) {
    uint32_t pos = playlist.position();
	// Считываем строку и если она валидна (CSV), записываем позицию в индекс
...
//        BOOTLOG("audio:\t\t%s (CS-%d, DCS-%d, DREQ-%d, RST-%d, SPI-%s, MOSI-%d, MISO-%d, SCK-%d)", "VS1053", VS1053_CS, VS1053_DCS, VS1053_DREQ, VS1053_RST, VS_HSPI?"HSPI":"VSPI", mosi_pin, miso_pin, sclk_pin);
  ...
  // ФИНАЛЬНЫЙ ОТЧЕТ ПО ПАМЯТИ И ЯДРАМ
